...
class PhantomxGymEnv(gym.Env):
    ...
    def __init__(self, urdf_root=pybullet_data.getDataPath(), 
                 ...
                 x_velocity_weight = 3*DT,# 26  #x方向速度跟随奖励项系数
                 y_velocity_weight = 3*DT,# 2   #y方向速度跟随奖励项系数
                 ...
                 action_rate = 0.5*1.0*DT, # 0.75 2.5        #action变化率惩罚项系数
                 yaw_weight = 3.0*DT, # 3          #yaw角度惩罚项系数
                 hard_reset=True,
    ...
    def step(self, action):
        ...
        # if self._env_step_counter == 94*2.5:
        #     self._set_goal_flag = False
        #     self._goal_state[0] = np.random.uniform(0.0, 0.25)
        #     self._goal_state[1] = np.random.uniform(-0.25, 0.25)
        ...
        # info = {}
        # collision = self.phantomx.GetCollisionWithGround()
        # print(collision)
    ...
    def reward_function(self, desired_x, current_x, yaw_flag):
        ...
        if yaw_flag:
            if current_x == desired_x:
                ...
                return 3.5 * (current_x - desired_x) + 1 #1
            else:
                return 3.5 * (-current_x + desired_x) + 1
        if current_x == desired_x:
            return 1
        elif current_x < desired_x:
    ...
    def penalty_function(self, desired_x, current_x, angvel_flag):
        # 一范数计算惩罚
        if angvel_flag:
            return -0.4 * abs(current_x - desired_x)
        return -3.5 * abs(current_x - desired_x)
    ...
    def _reward_veltrack(self, observations, action):
        ...
        # with open("x_velocity_average.txt", "a") as f:
        ...
        # 将x_average_velocity保存到文件中，保存后可以通过运行test.py可视化平均速度曲线
        # with open("y_velocity_average.txt", "a") as f:
        #     f.write(str(y_average_velocity) + "\n")
        ...
        # 将height_velocity保存到文件中，保存后可以通过运行test.py可视化平均速度曲线
        # with open("x_velocity.txt", "a") as f:
        #     f.write(str(current_base_velocity[2]) + "\n")
        ...
        # energy_reward = self.reward_function(0, energy_reward, True)

        # elif energy_reward < -5 / self._objective_weights[5]:
        #     energy_reward = -5 / self._objective_weights[5]

        # 将energy_reward保存到文件中，保存后可以通过运行test.py可视化平均速度曲线
        # with open("x_velocity.txt", "a") as f:
        #     f.write(str(energy_reward) + "\n")



        # pelnaty for action rate
        action_rate_reward = -np.abs(np.dot(delta_action, delta_action))
        if self._env_step_counter < LISTLEN:
            action_rate_reward /= 10
        if self._env_step_counter == 0:
        ...
        yaw_penalty = self.reward_function(aim_yaw, yaw, True)
        if self._env_step_counter == 0:
            yaw_penalty = 0
        # reward限幅到 > -5
        ...
        with open("yaw_velocity_average4.txt", "a") as f:
            f.write(str(yaw) + "\n")
        ...
        #     yaw_penalty /= 2
        # if abs(yaw-aim_yaw) > 0.1:
        #     print(self._env_step_counter)

        # print("aim_yaw:", aim_yaw)
        # print("yaw:", yaw)
